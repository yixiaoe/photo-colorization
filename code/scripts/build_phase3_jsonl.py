...
Usage (run from code/):
  python scripts/build_phase3_jsonl.py \
      ...
      --out_file       data/phase3_color_object_train.jsonl
"""
import argparse
import json
import os
...
def process_ann(coco: COCO, ann: dict, img_meta: dict, img_dir: str,
    ...
    """Return jsonl record dict or None if skipped."""
    if ann.get('iscrowd', 0):
        return None
    seg = ann.get('segmentation')
    if not seg or isinstance(seg, dict):
        # skip RLE-only (we want polygon for clean masks)
        return None
    ...
    # person and small objects use stricter area threshold
    ...
    try:
        ...
        return None
    if pil.size != (W, H):
        # COCO meta sometimes off; use actual size for cropping but skip mask alignment
        W, H = pil.size

    # rasterise mask via pycocotools
    try:
        mask_full = coco.annToMask(ann)        # (H, W) uint8
    except Exception:
        return None
    ...
    with open(args.out_file, 'w') as f:
        for n, ann in enumerate(anns):
            ...
            if im is None:
                skipped += 1
                continue
            rec = process_ann(coco, ann, im, args.img_dir, rng, args.min_area_ratio)
            if rec is None:
                skipped += 1
            else:
